...
const TopperCard = ({ name, percentage, achievement, rank, image, animationClass }) => (
  <div className={`w-[280px] rounded-2xl shadow-lg overflow-visible relative mt-28 ${animationClass}`}>
    {/* Overflowing Image */}
    <img 
      src={image}
      alt={name}
...
const TopperSection = ({ data }) => {
  // Animation classes for left, center, right
    ...
    <div className="py-12">
      <div className="flex justify-center items-center gap-8 flex-wrap">
        {data.map((topper, index) => (
          <TopperCard key={index} {...topper} animationClass={animationClasses[index] || ''} />
        ))}
      </div>
    </div>
  );
